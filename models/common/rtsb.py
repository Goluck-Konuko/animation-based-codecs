'''Modified from TIC (Lu et al., "Transformer-based Image Compression," DCC2022.)
    with simplification on the module configurations.'''
import torch
...
class MLP(nn.Module):
    '''MLP head for the Transformer block'''
    def __init__(self, in_features, hidden_features=None, out_features=None, drop=0.):
        super().__init__()
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
        self.mlp = nn.Sequential()
        self.mlp.add_module('norm', nn.LayerNorm(in_features))
        self.mlp.add_module('fc_1',nn.Linear(in_features, hidden_features))
        self.mlp.add_module('act',nn.GELU())
        self.mlp.add_module('fc_2', nn.Linear(hidden_features, out_features))
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim=3, window_size=(8,8), num_heads=2,use_prompt=False,num_prompts=1, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        ##separate q, k and v projections to allow for cross attention
    ...
    def forward(self, x,mask=None, query=None): #pass query token to perform cross attention
        """
        Args:
        ...
        if query != None:
            b_,n_,c_ = query.shape
            query = query.repeat(1,N//n_,1)
            q = self.query_proj(query) #cross attention
        else:
            q = self.query_proj(x) #self attention # maybe with a prompt mask
        k = self.key_proj(x)
        v = self.value_proj(x)
...
class RSTB(nn.Module):
    ...
    Args:
        ...
        use_checkpoint (bool): Whether to use checkpointing to save memory. Default: False.
    """
    def __init__(self, dim: int, input_resolution, depth:int, num_heads:int, window_size:int,
                 mlp_ratio:int=4., qkv_bias:bool=True, qk_scale:bool=None, drop:float=0., attn_drop:float=0.,
